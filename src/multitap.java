...
//        int elect_product[] = new int[K];
        ...
        for(int i = N; i < K; i++){
            for(int j = 0; j < N; j++){
                System.out.println("inner filter - elect_product[i] = " + elect_product.get(i) + " index i = " + i);
                System.out.println("inner filter - multitap_hole[j] = " + multitap_hole.get(j) + " index j = " + j);
